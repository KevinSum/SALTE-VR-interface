...
public class Print : MonoBehaviour {
	...
	void Start () {
        ...
        server.MessageDispatcher.AddCallback(
            "/soundVis", // OSC address
            (string address, OscDataHandle data) => {
                soundVis = Convert.ToBoolean(data.GetElementAsInt(0));
            }
        );
    ...
    IEnumerator PrintData()
    {
        ...
        while (true)
        {
            ...
            if (OVRInput.GetDown(OVRInput.RawButton.X) && Mathf.Abs(ele) < oscData.triggerRange) // Trigger print
            {
                sound_trigger_idx = 1;
            }
            else
            ...
            if (PauseScript.Paused == false && timer.gameOver == false && logData == true) // if the game is not paused
            {
