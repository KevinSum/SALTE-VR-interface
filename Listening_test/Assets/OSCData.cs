    ...
    void Update () {
        ...
        if (OVRInput.GetDown(OVRInput.RawButton.X) && Mathf.Abs(tracker.ele) < triggerRange && save_state == false)
        {
            ...
            if (TargetAzimuthArray.Length == 0) // if the array size is 0
            {
                ...
                while (Mathf.Abs(targetAzimuth) < 50); // Make sure new position is outside of view
            ...
            if (tracker.soundVis == true)
            {
                soundSourceVisTemp = true;
            }

            // Change sound source position
            client.Send("/changeSoundPos", targetAzimuth + azi_anchor, targetElevation + ele_anchor);
